...
namespace CodeRun.Services.Domain.Repository
{
    public class DapperRepository : IDapperRepository
    {
        ...
        {
            ValidateParameters(sql);
            LogOperation("多结果集查询", sql, param);

            // 多结果集查询通常较复杂，使用独立连接避免阻塞
            return await ExecuteWithIndependentConnection(async (conn) =>
                await conn.QueryMultipleAsync(sql, param, transaction, commandTimeout ?? _options.DefaultCommandTimeout, commandType));
        }
    }
}
